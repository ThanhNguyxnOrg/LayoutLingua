<#
.SYNOPSIS
    Build the Windows desktop app into dist\LayoutLingua and zip it for release.
...
$output = Join-Path $root "dist\LayoutLingua"
...
$running = Get-Process -Name "LayoutLingua" -ErrorAction SilentlyContinue |
...
# check the payload rather than trusting that the folder exists.
$required = @(
    "LayoutLingua.exe",
