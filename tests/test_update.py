        ...
        release = self._with_release(
            {
                "tag_name": "v0.3.0",
                "assets": [
                    {"name": "LayoutLingua-macos-intel.dmg", "browser_download_url": "mac", "size": 1},
                    {
                        "name": "LayoutLingua-windows.zip",
...
class StagingLayoutTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.install = Path(self.temp.name) / "LayoutLingua"
        self.install.mkdir()
...
class DownloadTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.destination = Path(self.temp.name) / "LayoutLingua-windows.zip"
        self.body = b"x" * (3 << 20)
...
class StageUpdateTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        self.install = self.root / "LayoutLingua"
        self.install.mkdir()
    ...
    def test_the_install_folder_is_never_touched_by_staging(self):
        (self.install / "LayoutLingua.exe").write_bytes(b"old")
        self._stage()
        self.assertEqual((self.install / "LayoutLingua.exe").read_bytes(), b"old")
...
class UpdaterScriptTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        self.install = write_payload(self.root / "LayoutLingua", b"old")
    ...
    def test_the_new_build_replaces_the_old_one_and_the_old_one_is_deleted(self):
        ...
        self.assertEqual((self.install / "LayoutLingua.exe").read_bytes(), b"new")
    ...
    def test_the_launcher_really_starts_the_helper_and_the_swap_happens(self):
        ...
        while time.monotonic() < deadline:
            try:
                ...
                swapped = (self.install / "LayoutLingua.exe").read_bytes() == b"new"
            except OSError:
                swapped = False
    ...
    def test_a_failed_swap_rolls_the_old_build_back(self):
        ...
        locked = (staged_payload(self.install) / "LayoutLingua.exe").open("ab")
        try:
            result = self._run()
        finally:
            locked.close()
