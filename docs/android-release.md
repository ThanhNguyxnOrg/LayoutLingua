...
keytool -genkeypair -v \
  -keystore layoutlingua-release.jks \
...
```properties
storeFile=/absolute/path/to/layoutlingua-release.jks
storePassword=…
keyAlias=layoutlingua
keyPassword=…
```
...
| Secret | Value |
| --- | --- |
| `ANDROID_KEYSTORE_BASE64` | `base64 -w0 layoutlingua-release.jks` |
| `ANDROID_KEYSTORE_PASSWORD` | the keystore password |
| `ANDROID_KEY_ALIAS` | `layoutlingua` |
...
`LayoutLingua-windows.zip` from whatever it finds there. A `v*` tag carrying
...
4. `.github/workflows/android-build.yml` checks the tag against the version,
   ...
   `LayoutLingua-android-<version>.apk`.
