...
ICONSET="$ICON_ROOT/LayoutLingua.iconset"
rm -rf "$ICONSET"
mkdir -p "$ICONSET"
...
iconutil -c icns "$ICONSET" -o "$ICON_ROOT/LayoutLingua.icns"
...
APP="$ROOT/dist/LayoutLingua.app"
EXECUTABLE="$APP/Contents/MacOS/LayoutLingua"
...
DMG="$ROOT/dist/LayoutLingua-macos-$ARCH_LABEL.dmg"
rm -rf "$DMG_STAGE"
mkdir -p "$DMG_STAGE"
cp -R "$APP" "$DMG_STAGE/"
...
for attempt in 1 2 3; do
  rm -f "$DMG"
  if hdiutil create -volname "LayoutLingua" -srcfolder "$DMG_STAGE" \
