...
to maximize translation yield while maintaining 100% fail-closed safety for damaged placeholders.
...
def normalize_mt_placeholders(text: str) -> str:
    ...
    4. Malformed style tags (< s 1 > -> <s1>, < / s 1 > -> </s>)

    Note: Truly unclosed or mismatched tags are intentionally NOT auto-closed,
    ...
    # Step 3: Remove spaces within style tags
    cleaned = SPACED_STYLE_OPEN.sub(r"<s\1>", cleaned)
    cleaned = SPACED_STYLE_CLOSE.sub(r"</s>", cleaned)
