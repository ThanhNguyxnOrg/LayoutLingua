...
def is_writable(directory: Path) -> bool:
    probe = directory / ".layoutlingua-write-test"
    try:
        probe.touch()
        probe.unlink()
        return True
...
$Backup = $Install + ".old"
$Executable = Join-Path $Install "LayoutLingua.exe"
...
function Test-Payload($root) {
    if (-not (Test-Path -LiteralPath (Join-Path $root "LayoutLingua.exe"))) { return $false }
...
def log_path() -> Path:
    ...
    base = Path(os.environ.get("LOCALAPPDATA") or Path.home()) / "LayoutLingua"
