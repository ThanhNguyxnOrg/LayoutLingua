...
STATUS_COLORS = {
    "queued": ("gray45", "#64748B"),
    "running": ("#0284C7", "#00F2FE"),
    "done": ("#059669", "#10B981"),
    "partial": ("#D97706", "#F59E0B"),
    "failed": ("#DC2626", "#EF4444"),
    "skipped": ("gray45", "#64748B"),
}
ACCENT = ("#0284C7", "#00F2FE")
MUTED = ("#64748B", "#94A3B8")
SURFACE = ("#F8FAFC", "#0F172A")
HOVER = ("#E2E8F0", "#1E293B")
BORDER_IDLE = ("#CBD5E1", "#334155")
...
class QueueRow:
    ...
    def __init__(self, parent, path: Path, app: App) -> None:
        ...
        self.frame = ctk.CTkFrame(
            ...
            border_color=BORDER_IDLE, fg_color=("white", "#131D31"),
        )
        self.frame.grid_columnconfigure(1, weight=1)

        self.mark = ctk.CTkLabel(
            self.frame, text=STATUS_MARKS["queued"], width=20,
            font=ctk.CTkFont(app.ui_font, size=14, weight="bold"),
            text_color=STATUS_COLORS["queued"],
        )
        self.mark.grid(row=0, column=0, padx=(PAD + 2, 0), pady=8)

        self.name = ctk.CTkLabel(
            self.frame, text=path.name, anchor="w", justify="left",
            font=ctk.CTkFont(app.mono_font, size=12, weight="bold"),
            text_color=("gray20", "#F8FAFC"),
        )
        self.name.grid(row=0, column=1, padx=PAD, pady=8, sticky="ew")

        self.detail = ctk.CTkLabel(
            self.frame, text="", anchor="e", width=96,
            font=ctk.CTkFont(app.mono_font, size=11), text_color=MUTED,
        )
        self.detail.grid(row=0, column=2, pady=8, sticky="e")
    ...
    def _enter(self, _event) -> None:
        self.frame.configure(fg_color=HOVER, border_color=ACCENT)

    def _leave(self, _event) -> None:
        self.frame.configure(fg_color=("white", "#131D31"), border_color=BORDER_IDLE)
...
class App(ctk.CTk, TkinterDnD.DnDWrapper):
    ...
    def _build_header(self) -> None:
        ...
        self.update_link = ctk.CTkLabel(
            right_box, text="", anchor="e", cursor="hand2", text_color=ACCENT,
            font=ctk.CTkFont(self.ui_font, size=12, weight="bold"),
        )
        self.update_link.pack(anchor="e")
    ...
    def _build_dropzone(self) -> None:
        ...
        self.dropzone_text = ctk.CTkLabel(
            self.dropzone, text="Drag and drop PDF files or folders here",
        ...
        buttons.grid(row=3, column=0, pady=(0, GAP))
        for text, command in (
            ("Select Files", self._pick_files),
            ("Select Folder", self._pick_directory),
        ):
            ctk.CTkButton(
    ...
    def _resize_dropzone(self) -> None:
        ...
        self.dropzone_text.configure(
            text="Drag more files here" if compact else "Drag and drop PDF files or folders here",
            font=ctk.CTkFont(self.ui_font, size=12 if compact else 14),
        )
        if compact:
            self.dropzone_glyph.grid_remove()
            self.dropzone_subtext.grid_remove()
        else:
            self.dropzone_glyph.grid()
            self.dropzone_subtext.grid()
    ...
    def _build_controls(self) -> None:
        ...
        ctk.CTkLabel(
            controls, text="Translate to", font=ctk.CTkFont(self.ui_font, size=13)
        ...
        self.translate_button = ctk.CTkButton(
            controls, text="Translate All", width=132, height=40, corner_radius=20,
            command=self._start, font=ctk.CTkFont(self.ui_font, size=14, weight="bold"),
            fg_color=("#0284C7", "#00F2FE"), text_color=("#FFFFFF", "#040812"),
        ...
        self.overwrite = ctk.CTkCheckBox(
            controls, text="Overwrite previously translated files",
    ...
    def _build_queue(self) -> None:
        ...
        self.empty_state = ctk.CTkLabel(
            ...
            text="No files queued.\nDrag and drop PDFs above to start.",
            font=ctk.CTkFont(self.ui_font, size=12),
        )
    ...
    def _build_footer(self) -> None:
        ...
        self.progress = ctk.CTkProgressBar(footer, height=6, progress_color=ACCENT)
        self.progress.set(0)
        self.progress.grid(row=0, column=0, pady=(0, PAD), sticky="ew")
        self.progress.grid_remove()

        self.status = ctk.CTkLabel(
            footer, text="No files queued", anchor="w", text_color=MUTED,
            font=ctk.CTkFont(self.ui_font, size=12),
        )
    ...
    def _show_output_link(self) -> None:
        if self.last_output is not None:
            self.output_link.configure(text=f"Open output directory:  {self.last_output}")
    ...
    def _refresh_queue_chrome(self) -> None:
        ...
        self.queue_title.configure(text=f"Queue ({count})" if count else "Queue")
        if count:
    ...
    def _install_update(self) -> None:
        ...
        if self.worker and self.worker.is_alive():
            self.status.configure(text="Translating — install update after completion")
            return
        install = install_directory()
        ...
        try:
            ...
            self._show_update_link(f"● Update available {self.update_tag}", "browser")
            self.status.configure(text="Could not launch updater — download manually")
            return
        self.destroy()
    ...
    def _confirm_install(self) -> bool:
        ...
        ctk.CTkLabel(
            dialog, text=f"Version {self.update_tag} downloaded", anchor="w",
            font=ctk.CTkFont(self.ui_font, size=15, weight="bold"),
        ).grid(row=0, column=0, padx=EDGE, pady=(EDGE, PAD), sticky="ew")

        ctk.CTkLabel(
            dialog, text="The app will close, install the update, and restart. Takes about 10 seconds.",
        ...
        ctk.CTkButton(
            buttons, text="Install & Restart", width=180, height=34, command=accept,
            font=ctk.CTkFont(self.ui_font, size=13),
        ).pack(side="right")
        ctk.CTkButton(
            buttons, text="Later", width=100, height=34,
            fg_color="transparent", border_width=1, border_color=BORDER_IDLE,
    ...
    def _start(self) -> None:
        ...
        self.status.configure(
            text=f"Preparing {pages} page(s)…" if pages else "Preparing…"
        )
    ...
    def _run(self, files: list[Path], language: str, overwrite: bool) -> None:
        for index, path in enumerate(files, 1):
            ...
            try:
                ...
                detail = (
                    f"{result.untranslated} segment(s) untranslated"
    ...
    def _log_failure(destination: Path, source: Path, error: BaseException) -> Path | None:
        ...
        try:
            ...
            path = destination / "layoutlingua.log"
        ...
        while True:
            ...
            if event[0] == "status":
                ...
                if state in ("failed", "skipped"):
                    ...
                    row.set_state(state, "", f"{first_line}  -  click for details")
                elif state == "partial":
                ...
                if self.states.get(path) == "running" and total:
                    ...
                    self.rows[path].detail.configure(text=f"page {done}/{total}")
                    self.progress.set((self.batch_done + done / total) / max(self.batch_total, 1))
                    self.status.configure(text=f"Translating {path.name}   page {done}/{total}")
                ...
                self._show_update_link(f"● Update available {event[1]}", "browser")
            elif event[0] == "update_downloading":
                self.update_tag = event[1]
                self._show_update_link(f"↓ Downloading {event[1]}…", None)
            elif event[0] == "update_progress":
                self._show_update_link(f"↓ Downloading {self.update_tag} — {event[1]}%", None)
                ...
                summary = f"Finished {counts.get('done', 0)}/{len(self.files)} file(s)"
